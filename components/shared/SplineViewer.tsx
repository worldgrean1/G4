...
const SplineViewer = memo(function SplineViewer({
  ...
  useEffect(() => {
    ...
    if (enableReset && typeof window !== 'undefined') {
      ...
      return () => window.removeEventListener('grean:reset-3d-view', handleResetEvent);
    }
  ...
  if (!scene) {
    return (
      <div className={`spline-error-container flex items-center justify-center p-8 ${className}`}>
        <div className="text-center">
          <div className="text-red-500 mb-2">⚠️</div>
          <h3 className="text-lg font-semibold text-gray-800 mb-2">Configuration Required</h3>
          <p className="text-sm text-gray-600">
            Spline scene URL is not configured. Please set NEXT_PUBLIC_SPLINE_SCENE in your .env.local file
            or provide a scene prop.
          </p>
        </div>
      </div>
    );
