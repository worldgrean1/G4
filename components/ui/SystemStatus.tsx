...
import { getAudioEnabled, getAudioVolume, setAudioEnabled } from '@/utils/sound';
...
export function SystemStatusTooltip({ className = '' }: { className?: string }) {
  const { animationsPaused, inverterActive, switchActive } = useEnergySystemStore();
  ...
  const statusItems = [
    ...
    },
    {
      icon: Palette,
      label: 'Theme',
      value: theme === 'system' ? `System (${effectiveTheme})` : theme,
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      >
        <Info size={14} className="text-gray-300 hover:text-white transition-colors" />
      </motion.button>
            ...
            <div className="bg-black/85 backdrop-blur-sm border border-gray-700/60 rounded-lg p-2.5 shadow-xl min-w-52">
              <div className="space-y-2.5">
                ...
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-semibold text-white tracking-wide">System Status</h3>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 space-y-0.5">
                          ...
                          <div className={`text-[10px] font-semibold ${item.color} leading-none`}>
                            {item.value}
                          </div>
                        </div>
                ...
                <div className="border-t border-gray-700/50 pt-2">
                  <div className="text-[9px] text-gray-400 mb-1.5 font-medium uppercase tracking-wider">Components</div>
                        ...
                        <div className="flex items-center justify-center w-3 h-3">
                          <div
                            className={`w-2 h-2 rounded-full ${
                              component.active ? 'bg-current animate-pulse shadow-sm' : 'bg-gray-600'
                            }`}
                          />
                        </div>
                ...
                <div className="border-t border-gray-700/50 pt-2">
                  <div className="text-[9px] text-gray-400 mb-1.5 font-medium uppercase tracking-wider">Shortcuts</div>
                  <div className="grid grid-cols-2 gap-1.5">
                    <div className="flex items-center justify-between bg-gray-800/40 px-1.5 py-1 rounded border border-gray-700/30">
                      <span className="text-[9px] text-gray-400 font-medium">Reset:</span>
                      <kbd className="text-[7px] bg-gray-700/80 px-1.5 py-0.5 rounded text-gray-300 font-mono border border-gray-600/50">R</kbd>
                    </div>
                    <div className="flex items-center justify-between bg-gray-800/40 px-1.5 py-1 rounded border border-gray-700/30">
                      <span className="text-[9px] text-gray-400 font-medium">Exit:</span>
                      <kbd className="text-[7px] bg-gray-700/80 px-1.5 py-0.5 rounded text-gray-300 font-mono border border-gray-600/50">Esc</kbd>
                    </div>
                  </div>
                </div>
              </div>
    ...
    >
      <div className="bg-black/95 backdrop-blur-sm border border-gray-600/40 rounded-md overflow-hidden shadow-lg">
        ...
        <div className="flex flex-col w-3 bg-gray-900/90">
          <div className={`h-1.5 ${animationsPaused ? 'bg-yellow-400' : 'bg-green-400'} ${!animationsPaused ? 'animate-pulse' : ''}`} />
          <div className={`h-1.5 ${inverterActive ? 'bg-blue-400' : 'bg-gray-700'} ${inverterActive ? 'animate-pulse' : ''}`} />
          <div className={`h-1.5 ${switchActive ? 'bg-green-400' : 'bg-gray-700'} ${switchActive ? 'animate-pulse' : ''}`} />
          <div className={`h-1.5 ${audioEnabled ? 'bg-cyan-400' : 'bg-gray-700'}`} />
          <div className={`h-1.5 ${effectiveTheme === 'dark' ? 'bg-purple-400' : 'bg-orange-400'}`} />
        </div>
            ...
            <div className="p-2 space-y-1 w-32">
              ...
              <div className="space-y-0.5">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className={`text-xs ${animationsPaused ? 'text-yellow-400' : 'text-green-400'}`}>
                ...
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-1">
                    <div className={`w-1.5 h-1.5 rounded-full ${inverterActive ? 'bg-blue-400 animate-pulse' : 'bg-gray-600'}`} />
                    <span className="text-gray-400 text-xs">Inv</span>
                  </div>
                  <span className={`text-xs ${inverterActive ? 'text-blue-400' : 'text-gray-500'}`}>
                ...
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-1">
                    <div className={`w-1.5 h-1.5 rounded-full ${switchActive ? 'bg-green-400 animate-pulse' : 'bg-gray-600'}`} />
                    <span className="text-gray-400 text-xs">Sw</span>
                  </div>
                  <span className={`text-xs ${switchActive ? 'text-green-400' : 'text-gray-500'}`}>
                    {switchActive ? 'On' : 'Off'}
                  </span>
                </div>
                ...
                >
                  ...
                  <span className={`text-xs ${audioEnabled ? 'text-cyan-400' : 'text-gray-500'}`}>
                    {audioEnabled ? 'On' : 'Off'}
                  </span>
                </div>
              </div>
