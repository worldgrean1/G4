...
export class AudioGenerator {
  ...
  async generateTyping(): Promise<void> {
    ...
    try {
      ...
      console.warn('Failed to generate typing sound:', error);
    }
  }
  ...
  async generateAlarm(): Promise<void> {
    ...
    try {
      ...
      console.warn('Failed to generate alarm:', error);
    }
  }
  ...
  async generateMorningSunshine(): Promise<void> {
    ...
    try {
      ...
      const windBuffer = ctx.createBuffer(1, ctx.sampleRate * duration, ctx.sampleRate);
      ...
      sunriseOsc.frequency.exponentialRampToValueAtTime(440, currentTime + duration);

      sunriseFilter.type = 'lowpass';
      sunriseFilter.frequency.setValueAtTime(1000, currentTime);
      sunriseFilter.frequency.linearRampToValueAtTime(2000, currentTime + duration);
