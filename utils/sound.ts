...
function saveAudioPreferences(preferences: AudioPreferences): void {
  ...
  try {
    localStorage.setItem('grean-audio-preferences', JSON.stringify(preferences));
  } catch (error) {
...
function createSound(srcArray: string[], volume: number = 0.3): Howl {
  return new Howl({
    ...
    onload: () => {
      ...
      console.log(`Sound loaded successfully: ${srcArray[0]}`);
    },
    onloaderror: (id, error) => {
      const key = srcArray[0].split('/').pop()?.split('.')[0] || 'unknown';
      soundLoadStatus[key] = 'error';
      console.warn(`Failed to load sound: ${srcArray[0]}`, error);
    },
  });
}
...
async function playSound(soundKey: keyof typeof soundEffects): Promise<void> {
  ...
  try {
    ...
    if (!soundInstances[soundKey]) {
      soundInstances[soundKey] = createSound(soundEffects[soundKey], preferences.volume);
      soundLoadStatus[soundKey] = 'loading';
    }

    // Check if the sound file loaded successfully
    const soundFile = soundEffects[soundKey][0].split('/').pop()?.split('.')[0] || soundKey;
...
async function playFallbackSound(soundKey: keyof typeof soundEffects): Promise<void> {
  ...
  try {
    ...
    console.log(`Played fallback sound for: ${soundKey}`);
  } catch (error) {
    console.warn(`Failed to play fallback sound for ${soundKey}:`, error);
  }
}

// Exported functions
export function playTypingSound(): void {
  playSound('typing').catch(console.error);
}

export function playButtonClickSound(): void {
  playSound('buttonClick').catch(console.error);
}

export function playAlarmSound(): void {
  playSound('alarm').catch(console.error);
}

export function playMorningSunshineSound(): void {
  playSound('morningSunshine').catch(console.error); // Using magic-sunshine.m4a
}

// Debug function to get sound status
export function getSoundStatus(): { [key: string]: string } {
  return { ...soundLoadStatus };
}

// Force fallback mode for testing
export async function testFallbackSounds(): Promise<void> {
  const audioGen = getAudioGenerator();
  ...
  try {
    ...
    console.log('Playing morning sunshine...');
    await audioGen.generateMorningSunshine();

    console.log('Fallback sound test completed!');
  } catch (error) {
    console.error('Error during fallback sound test:', error);
  }
}
...
export async function runAudioDiagnostics(): Promise<void> {
  ...
  console.log('Testing fallback audio generation...');
  await testFallbackSounds();
